...
public class Teste {
	...
	public static void main(String [] args){
		...
		// int player.getState() = ACTIVE;						// estado
		// double player.getX() = GameLib.WIDTH / 2;					// coordenada x
		// double player.getY() = GameLib.HEIGHT * 0.90;				// coordenada y
		// double player_VX = 0.25;						// velocidade no eixo x
		// double player_VY = 0.25;						// velocidade no eixo y
		// double player.getRadius = 12.0;						// raio (tamanho aproximado do player)
		// double player.getExplosion_start() = 0;					// instante do início da explosão
		// double player.getExplosion_end() = 0;					// instante do final da explosão
		// long player.getNextShot() = currentTime;					// instante a partir do qual pode haver um próximo tiro

		State playerState = State.ACTIVE;   
		ArrayList <State> CircleEnemyState = new ArrayList<>();
		ArrayList <State> DiamondEnemyState = new ArrayList<>();
		ArrayList <State> PlayerProjectileState = new ArrayList<>();
		ArrayList <State> EnemyProjectileState = new ArrayList<>();
		
		
		
		Player player = new Player(GameLib.WIDTH / 2, GameLib.HEIGHT * 0.90, 0.25, 0.25, currentTime, 12.0);
		...
		while(running){
			...
			if(playerState.equals(State.ACTIVE)){
